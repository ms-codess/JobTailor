...
2.  **Generate Tailored Resume**: Adapt the user's existing resume to better match the job description. Do NOT remove the user's original tasks in the experience section. Instead, realistically adapt them by subtly integrating the missing skills. Only add keywords compatible with the user's existing experience. The output for 'tailoredResume' MUST be a structured JSON object. Each task/bullet point in the experience description must start with '- ' and be on a new line.
...
const skillAnalysisPrompt = ai.definePrompt({
    ...
    prompt: `You are an expert resume analyst. Analyze the provided resume against the job description.
...
2.  **Report on Integrated vs. Missing Keywords**: Assume a colleague has already created a 'tailored resume'. Based on the original resume and the job description, create a list of 'integratedKeywords' (skills that could be realistically added) and 'missingKeywords' (skills that represent a genuine gap).
